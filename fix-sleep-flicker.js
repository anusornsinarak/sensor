...
void drawUI() {
  ...
  tft.fillRoundRect(220, 198, 96, 38, 5, COLOR_BTN_SLEEP);
  tft.drawRoundRect(220, 198, 96, 38, 5, COLOR_CARD_LINE);
...
void setup() {
  ...
  digitalWrite(TFT_BL, HIGH); // เปิดไฟหน้าจอ
...
void loop() {
  // 1. ระบบสัมผัสหน้าจอแบบมี Debounce ป้องกันการกะพริบ
  if (millis() - lastTouchTime > 400) {
    if (touch.touched()) {
      ...
      if (isScreenSleep) {
        // --- 🌅 ปลุกหน้าจอ: แตะที่ตำแหน่งใดก็ได้บนหน้าจอขณะหลับอยู่ ---
        isScreenSleep = false;
        digitalWrite(TFT_BL, HIGH); // เปิดไฟจอให้สว่าง
        
        // รอให้นิ้วปล่อยออกจากหน้าจอก่อน
        ...
        // --- 🌙 สั่งหลับ (SLEEP): เมื่อแตะปุ่ม SLEEP มุมขวาล่าง ---
        if ((screenX >= 210 && screenY >= 185) || (p.x > 2400 && p.y > 2400)) {
          isScreenSleep = true;
          
          // รอให้นิ้วปล่อยออกจากหน้าจอก่อน ดับไฟจอ เพื่อไม่ให้เกิดลูปกะพริบ
          while (touch.touched()) { delay(20); }
          delay(100);
          
          digitalWrite(TFT_BL, LOW); // ดับไฟหน้าจอ 100% มืดสนิท
          lastTouchTime = millis();
        }
      }
    }
  ...
  // 3. อัปเดตเวลา (ทุก 1 วินาที - ถ้าหลับอยู่จะไม่รันคำสั่งวาดจอ)
...
console.log('Successfully fixed sleep flicker and wake-up mechanism!');
